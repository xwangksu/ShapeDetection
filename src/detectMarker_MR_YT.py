...
# Set working path
workingPath = "F:/Xu/YT_test/BW_YT_1-36_Rededge_0309/"
# Set source images path
srcImagePath = workingPath+"blue/"
imageFiles = os.listdir(srcImagePath)
# Set GCP file
srcGCPFile = "F:/Xu/YT_test/17BW_TY_1-36_GCPs_2.csv"
...
finalFile = open(workingPath+"markerList"+t_index+".csv",'wt')
...
#         cv2.waitKey(0)
        ...
        for c in cnts:
            ...
            shape = sd.detect(c)
#             print(shape)
            if cv2.contourArea(c)>=contour_min_size and cv2.contourArea(c)<=contour_max_size and shape == "rectangle": # or shape == "rectangle"):
                ...
                if M["m00"] != 0:
                    ...
                    cX = 0
                    cY = 0
                if cX != 0 and cY != 0 and (cX>int(img_width*0.25) and cX<int(img_width*0.75)) and (cY>int(img_height*0.25) and cY<int(img_height*0.75)):
                    snum = snum+1
        if snum == 1:
            ...
            if cX != 0 and cY != 0 and (cX>int(img_width*0.25) and cX<int(img_width*0.75)) and (cY>int(img_height*0.25) and cY<int(img_height*0.75)):
                ...
                thDist = 1.04/contourSize*pixelDist2ImgCenter
