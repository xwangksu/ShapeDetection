...
#         print(numpy.sqrt(numpy.sum((pointCamera - point_gcp) ** 2)) - theDist)
    ...
    if numpy.abs(minDist - secondMinDist) >= 15:
...
#             [utmGCPx, utmGCPy, latGCPzone,longGCPzone] = utm.from_latlon(gcp[2],gcp[1])
#             point_gcp = numpy.array((utmGCPx, utmGCPy))
#             if numpy.sqrt(numpy.sum((pointCamera - point_gcp) ** 2)) - theDist <= 1.5:
#                 gcpLongi = gcp[1]
#                 gcpLati = gcp[2]
#                 gcpAlti = gcp[3]
#                 gcpLabel = gcp[0]
...
# Set working path
workingPath = "D:/Test_AP_MR_py/"
# Set source images path
srcImagePath = "D:/Test_AP_MR_py/Training_set1/"
imageFiles = os.listdir(srcImagePath)
# Set GCP file
srcGCPFile = "D:/Test_AP_py/2017_RF_BYD_GCPs.csv"
...
#         cv2.waitKey(0)
        ...
        for c in cnts:
            ...
            shape = sd.detect(c)
#             print(shape)
            if cv2.contourArea(c)>=contour_size and shape == "square": # or shape == "rectangle"):
        ...
        if snum == 1:
            ...
            for c in cnts:
                ...
                shape = sd.detect(c)
                if contourSize>=contour_size and shape == "square": # or shape == "rectangle"):
                    break
            # Matching GCPs
            M = cv2.moments(c)
            if M["m00"] != 0:
